...
## 2.How is inadequate googsleeptime correlated with heart attack?  
...
## 3.How is income level correlated with BMI categories (i.e. underweight, obese, ect.)?  
Often times, there are reports claiming that sudden death of young people is associated with inadequate sleep time. Since most sudden death of young people are caused by heart attack, it is interesting to investigate whether the heart attack history is associated with inadequate sleep time in young people (age 18 ~ 34). To compare, the data from elder group (age 55 and above) will also be explored.
...
data <- as.data.frame(t(apply(moneyweight_table, 1, function(i) i/sum(i))))
data
```

```{r, fig.height = 5, fig.width = 6, message = FALSE}
library(reshape2)
data2<- melt(cbind(income_level= rownames(data),data), id = 'income_level')
data2$income_level <- factor(data2$income_level, levels = c('Less than $15,000','$15,000 to less than $25,000', '$25,000 to less than $35,000', '$35,000 to less than $50,000', '$50,000 or more'))
